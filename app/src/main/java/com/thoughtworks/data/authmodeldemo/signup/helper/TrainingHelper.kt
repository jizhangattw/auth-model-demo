...
class TrainingHelper {
    fun start(context: Context): Flowable<Boolean> {
        return collectData(10, context)
            .recordData()
            .map {
                writeToFile("before_normalized.json", Gson().toJson(it), context)
                it
            }
            .normalized()
            .map {
                writeToFile("after_normalized.json", Gson().toJson(it), context)
                it
            }
            .reshapeData(25, 25)
    ...
    private fun collectData(sampling: Int, context: Context): Flowable<SensorData> {
    ...
    private fun Flowable<Array<FloatArray>>.reshapeData(windowWidth: Int, windowHeight: Int): Flowable<Array<Array<Array<Float>>>> {
        return map {
            val python = Python.getInstance()
